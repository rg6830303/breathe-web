...
type CellStatus = "open" | "booked" | "blocked" | "past";
...
export function LiveAvailability() {
  ...
  useEffect(() => {
    let active = true;
    const load = () => {
      // DB-safety: never poll from a hidden tab.
    ...
    load();
    // 60s + visibility-gated keeps it live without pressuring the free-tier
    // DB pool (each refresh is a single /api/slots call → 2 indexed queries).
